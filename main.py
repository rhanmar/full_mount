...
from utils.parsers.parser import ParserService
...
@app.get("/api/next_event/", response_model=EventRead)
def get_next_event(db: Session = Depends(get_db)):
    data = db.query(Event).order_by(Event.date.desc()).first()
    return data


@app.get("/api/fights/", response_model=list[FightRead])
def get_all_fights(db: Session = Depends(get_db)) -> list[FightRead]:
    data = db.query(Fight).join(Event).all()
    return data


@app.get("/api/fights/{fight_id}/", response_model=FightRead)
def get_fights_by_id(fight_id: int, db: Session = Depends(get_db)) -> FightRead:
...
#     from utils.parsers.parser import ParserService
#     service = ParserService()
#     return service.parse_all_events()
#     # return parse_events()
#
#
